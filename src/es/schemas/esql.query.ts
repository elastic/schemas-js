...
export const EsqlEsqlApproximationSettings = z.object({
  rows: integer.describe('The number of sampled rows used for approximating the query. It must be at least 10,000. A null value uses the system default.').optional(),
  confidence_level: double.describe('The confidence level of the computed confidence intervals. A null value disables computing confidence intervals.').optional()
...
export const EsqlEsqlQuerySettings = z.object({
  ...
  column_metadata: SpecUtilsStringified.describe('When enabled, column metadata is added to the query response as additional `_meta` properties. Currently, only `_meta.bucket` is added for columns corresponding to the `BUCKET` function, containing the bucket interval and unit for queries where it can be determined.').optional(),
  project_routing: ProjectRouting.describe('Limits the scope of a cross-project search (CPS) to specific projects before query execution, based on a Lucene query expression evaluated against project tags. Excluded projects are not queried, which can reduce cost and latency.').optional()
