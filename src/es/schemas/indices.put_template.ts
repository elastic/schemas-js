...
export const MappingSemanticTextProperty = z.object({
  ...
  inference_id: Id.describe('Inference endpoint that will be used to generate embeddings for the field. This parameter cannot be updated. Use the Create inference API to create the endpoint. If `search_inference_id` is specified, the inference endpoint will only be used at index time. If the `inference_id` is not specified, it will default to `.jina-embeddings-v5-text-small` if the cluster is authorized to use the Elastic Inference Service, otherwise it will default to `.elser-2-elasticsearch`. The `.elser-2-elasticsearch` inference endpoint relies on a local ML node to run the ELSER model.').optional(),
...
export const MappingDocValuesPropertyBase = z.object({
  ...
  copy_to: Fields.optional(),
  store: z.boolean().optional(),
  doc_values: MappingDocValues.optional()
...
export const IndicesIndexSettingsUnassignedNodeLeft = z.object({
  delayed_timeout: Duration.describe('The amount of time to wait for a node that has left before assuming its shards are permanently missing and starting to allocate replacement replicas.').optional()
...
export const IndicesIndexSettingsTimeSeries = z.object({
  ...
  temporality_field: Field.describe('The name of the field that stores the temporality of a metric. The referenced field must be a `keyword` dimension field; if the setting is unset or the field is missing or invalid, the metric temporality resolves to null.').optional()
