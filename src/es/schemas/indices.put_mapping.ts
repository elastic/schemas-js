...
import { DateTime, Duration, ExpandWildcards, Field, FieldValue, Fields, GeoLocation, Id, IndexName, Indices, IndicesResponseBase, Metadata, Name, PropertyName, RelationName, Script, ScriptSource, TokenPruningConfig, byte, double, float, integer, long, ulong } from './_types.js'
...
export interface MappingRuntimeFieldShape {
  ...
  fetch_fields?: Array<MappingRuntimeFieldFetchFields | Field> | undefined
...
export const MappingRuntimeField = z.object({
  ...
  fetch_fields: z.array(z.union([MappingRuntimeFieldFetchFields, z.lazy(() => Field)])).describe('For type `lookup`').optional(),
...
export const MappingDocValuesConfig = z.object({
  multi_value: z.boolean().describe('If `false`, the field is treated as single-valued, enabling optimized storage. Only has an effect when columnar index mode is active.').optional(),
  nullability: z.boolean().describe('If `false`, every document must provide a non-null value for the field: a document that omits the field, sets it to `null`, or supplies only null values (an empty array or an all-null array) is rejected at index time. A field that defines `null_value` is always exempt, since the configured default removes the absence of a value. Only has an effect when columnar index mode is active.').optional()
}).meta({ id: 'MappingDocValuesConfig' })
export type MappingDocValuesConfig = z.infer<typeof MappingDocValuesConfig>
...
export const MappingDocValues = z.union([z.boolean(), MappingDocValuesConfig]).meta({ id: 'MappingDocValues' })
export type MappingDocValues = z.infer<typeof MappingDocValues>
...
export const MappingTextIndexPrefixes = z.object({
  max_chars: z.lazy(() => integer).optional(),
  min_chars: z.lazy(() => integer).optional()
...
export const MappingPreserveLeafArrays = z.enum(['lossy', 'exact']).meta({ id: 'MappingPreserveLeafArrays' })
export type MappingPreserveLeafArrays = z.infer<typeof MappingPreserveLeafArrays>
...
export const MappingSemanticTextProperty = z.object({
  ...
  inference_id: z.lazy(() => Id).describe('Inference endpoint that will be used to generate embeddings for the field. This parameter cannot be updated. Use the Create inference API to create the endpoint. If `search_inference_id` is specified, the inference endpoint will only be used at index time. If the `inference_id` is not specified, it will default to `.jina-embeddings-v5-text-small` if the cluster is authorized to use the Elastic Inference Service, otherwise it will default to `.elser-2-elasticsearch`. The `.elser-2-elasticsearch` inference endpoint relies on a local ML node to run the ELSER model.').optional(),
...
export const MappingByteNumberProperty = z.object({
  ...
  time_series_dimension: z.boolean().describe('For internal use by Elastic only. Marks the field as a time series dimension. Defaults to false.').optional(),
  type: z.literal('byte'),
  null_value: z.lazy(() => byte).optional()
...
export interface MappingFlattenedPropertyShape {
  ...
  preserve_leaf_arrays?: MappingPreserveLeafArrays | undefined
...
export const MappingFlattenedProperty = z.object({
  ...
  preserve_leaf_arrays: MappingPreserveLeafArrays.describe('How leaf arrays are represented in synthetic source. When set to `lossy`, leaf arrays are sorted, de-nulled, and deduplicated in the returned synthetic source. When set to `exact`, leaf arrays preserve order, nulls, and duplicates.').optional(),
...
export const MappingJoinProperty = z.object({
  ...
  get fields (): z.ZodOptional<z.ZodRecord<typeof PropertyName, typeof MappingProperty>> { return z.record(PropertyName, MappingProperty).optional() },
  synthetic_source_keep: z.lazy(() => MappingSyntheticSourceKeepEnum).optional(),
  relations: z.record(z.lazy(() => RelationName), z.union([z.lazy(() => RelationName), z.array(z.lazy(() => RelationName))])).optional(),
  eager_global_ordinals: z.boolean().optional(),
  type: z.literal('join')
