...
export const InferenceCustomTaskSettings = z.object({
  parameters: z.record(z.string(), InferenceCustomTaskParameter).describe('Specifies parameters that are required to run the custom service. The parameters depend on the model your custom service uses. For example: ``` "task_settings":{   "parameters":{     "input_type":"query",     "return_token":true   } } ``` > warn > The `task_settings.parameters` cannot contain the same keys as `secret_parameters`. If they do, an error will be returned. This applies to PUT requests and POST requests.').optional()
