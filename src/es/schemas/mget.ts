...
import { ErrorCause, Fields, Id, Ids, IndexName, Routing, VersionNumber, VersionType } from './_types.js'
import { GetGetResult } from './get.js'
...
export const MgetRequest = z.object({
  ...
  routing: z.lazy(() => Routing).describe('Custom value used to route operations to a specific shard. Not allowed when `index.slice.enabled` is `true` for the target index; use `_slice` instead.').optional().meta({ found_in: 'query' }),
