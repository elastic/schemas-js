...
import { ByteSize, Duration, DurationValue, Name, integer, long } from './_types.js'
...
export const NodesProcessor = z.object({
  ...
  time: z.lazy(() => Duration).describe('Time spent by the processor transforming documents.').optional(),
...
export const NodesIngestStats = z.object({
  ...
  time: z.lazy(() => Duration).describe('Total time spent preprocessing ingest documents during the lifetime of this node.').optional(),
  ...
  ingested_as_first_pipeline: z.lazy(() => ByteSize).describe('Total size of all documents ingested by the pipeline. The value counts documents for which this pipeline was the first to process them; for pipelines that are not the first to process a document (for example, a final pipeline after a default pipeline, a pipeline run after a reroute processor, or a pipeline invoked by a pipeline processor), the value is `0`.').optional(),
  ingested_as_first_pipeline_in_bytes: z.lazy(() => long).describe('Total number of bytes of all documents ingested by the pipeline. The value counts documents for which this pipeline was the first to process them; for pipelines that are not the first to process a document, the value is `0`.'),
  produced_as_first_pipeline: z.lazy(() => ByteSize).describe('Total size of all documents produced by the pipeline. The value counts documents for which this pipeline was the first to process them; for pipelines that are not the first to process a document, the value is `0`. In situations where there are subsequent pipelines, the value represents the size of the document after all pipelines have run.').optional(),
  produced_as_first_pipeline_in_bytes: z.lazy(() => long).describe('Total number of bytes of all documents produced by the pipeline. The value counts documents for which this pipeline was the first to process them; for pipelines that are not the first to process a document, the value is `0`. In situations where there are subsequent pipelines, the value represents the size of the document after all pipelines have run.')
