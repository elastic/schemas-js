...
import { AcknowledgedResponseBase, ByteSize, Duration, Field, Fields, GeoShapeRelation, GrokPattern, Id, Metadata, Name, Script, ScriptLanguage, ScriptSource, SortOrder, VersionNumber, double, integer, long } from './_types.js'
...
export const IngestAttachmentProcessor = z.object({
  ...
  max_field_bytes: z.lazy(() => ByteSize).describe('Maximum allowed size of the attachment `field` value in bytes: length of a string (if base64 in JSON, checked before base64 decoding) or byte array length for binary (for example, CBOR). If set to `-1`, there is no per-processor limit. The node setting `ingest.attachment.max_field_size` also applies.').optional(),
...
export const IngestEnrichProcessor = z.object({
  ...
  shape_relation: z.lazy(() => GeoShapeRelation).describe('A spatial relation operator used to match the geoshape of incoming documents to documents in the enrich index. This option is only used for `geo_match` enrich policy types.').optional(),
...
export const IngestSortProcessor = z.object({
  ...
  order: z.lazy(() => SortOrder).describe('The sort order to use. Accepts `"asc"` or `"desc"`.').optional(),
...
export const IngestPutPipelineRequest = z.object({
  ...
  field_access_pattern: z.lazy(() => IngestFieldAccessPattern).describe('Controls how processors in this pipeline should read and write data on a document\'s source.').optional().meta({ found_in: 'body' })
