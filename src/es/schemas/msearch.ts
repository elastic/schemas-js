...
import { ErrorCause, ExpandWildcards, Indices, ProjectRouting, Routing, SearchType, integer, long } from './_types.js'
import { SearchResponseBody, SearchSearchRequestBody } from './search.js'
...
export const MsearchMultisearchHeader = z.object({
  ...
  routing: z.lazy(() => Routing).describe('A custom value used to route operations to a specific shard. Not allowed when `index.slice.enabled` is `true` for the target index; use `_slice` instead.').optional(),
...
export const MsearchRequest = z.object({
  ...
  routing: z.lazy(() => Routing).describe('Custom routing value used to route search operations to a specific shard. Not allowed when `index.slice.enabled` is `true` for the target index; use `_slice` instead.').optional().meta({ found_in: 'query' }),
